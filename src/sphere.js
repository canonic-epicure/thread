...
function createSphereTexture(renderer, letterColor, gridColor, fontFamily, text) {
...
export function createSphereController(options) {
    const { renderer, camera, materialParams, letterColor, gridColor, fontFamily, text } = options;
    let currentLetterColor = letterColor;
    let currentGridColor = gridColor;
    let currentFontFamily = fontFamily;
    let currentText = text;
    let texture = createSphereTexture(renderer, currentLetterColor, currentGridColor, currentFontFamily, currentText);
    ...
    const rebuildTexture = () => {
        const nextTexture = createSphereTexture(renderer, currentLetterColor, currentGridColor, currentFontFamily, currentText);
    ...
    renderer.domElement.addEventListener('pointermove', (event) => {
        ...
        const impulse = THREE.MathUtils.clamp(velocity * sphereDragSpeedFactor, -sphereMaxDragSpeed, sphereMaxDragSpeed);
        currentScrollSpeed = THREE.MathUtils.clamp(currentScrollSpeed + impulse, -sphereMaxDragSpeed, sphereMaxDragSpeed);
    });
    ...
    const updateSphere = (delta) => {
        sphere.rotation.y = 0;
        sphere.rotation.x = 0;
        targetScrollSpeed = currentBaseSpeed;
        const timeConstant = targetScrollSpeed === 0 ? sphereSlowdownTime : sphereResumeTime;
