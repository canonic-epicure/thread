...
function* createSpiralCharGenerator(chars) {
    if (chars.length === 0) {
        while (true) {
            yield ' ';
        }
...
function toSafeChars(text) {
    if (!text) {
        return [' '];
    }
    return Array.from(text);
}
function sampleGaussianOffset(stdDev, min, max) {
    let u = 0;
    let v = 0;
    while (u === 0)
        u = Math.random();
...
function createGlyphAtlas(chars, fontFamily) {
...
function createSpiralSlots(generator) {
    return Array.from({ length: SPIRAL_LETTER_COUNT }, (_, index) => ({
        char: generator.next().value ?? ' ',
        originalIndex: index,
        alteredIndex: index +
            sampleGaussianOffset(SPIRAL_OFFSET_STD_DEV, -SPIRAL_STRING_OFFSET_RADIUS, SPIRAL_STRING_OFFSET_RADIUS),
        displacedIndex: 0
    }));
}
function assignDisplacedIndices(slots) {
...
function createDepressedPlane(materialParams, planeColor) {
...
function createPlaneAlphaMap() {
    ...
    const center = size / 2;
    const radius = center;
    const gradient = ctx.createRadialGradient(center, center, radius * PLANE_FADE_INNER, center, center, radius * PLANE_FADE_OUTER);
...
function createPlaneParticles(half) {
    const particles = [];
    ...
    const spawnParticle = (particle) => {
        ...
        particle.pos.set(THREE.MathUtils.randFloatSpread(range * 2), THREE.MathUtils.randFloatSpread(range * 2));
        const angle = Math.random() * Math.PI * 2;
        particle.vel.set(Math.cos(angle), Math.sin(angle));
        particle.speed = THREE.MathUtils.lerp(PLANE_PARTICLE_SPEED_MIN, PLANE_PARTICLE_SPEED_MAX, Math.random());
        particle.age = 0;
        particle.life = THREE.MathUtils.lerp(PLANE_PARTICLE_LIFE_MIN, PLANE_PARTICLE_LIFE_MAX, Math.random());
    };
...
export function createSpiralController(options) {
    ...
    glyphAtlas.texture.anisotropy = Math.min(8, options.renderer.capabilities.getMaxAnisotropy());
    ...
    spiralGeometry.setAttribute('aOriginalT', new THREE.InstancedBufferAttribute(originalTArray, 1));
    spiralGeometry.setAttribute('aDisplacedT', new THREE.InstancedBufferAttribute(displacedTArray, 1));
    ...
    const spawnLens = (lens, randomAge) => {
        const angle = Math.random() * Math.PI * 2;
        lens.pos.set(THREE.MathUtils.randFloatSpread(lensBounds), THREE.MathUtils.randFloatSpread(lensBounds));
        lens.vel.set(Math.cos(angle), Math.sin(angle));
        lens.radius = THREE.MathUtils.lerp(PLANE_LENS_RADIUS_MIN, PLANE_LENS_RADIUS_MAX, Math.random());
        lens.baseStrength = THREE.MathUtils.lerp(PLANE_LENS_STRENGTH_MIN, PLANE_LENS_STRENGTH_MAX, Math.random());
        lens.baseSpeed = THREE.MathUtils.lerp(PLANE_LENS_SPEED_MIN, PLANE_LENS_SPEED_MAX, Math.random());
    ...
    const lensPosUniform = spiralMaterial.uniforms.uLensPos.value;
    const lensRadiusUniform = spiralMaterial.uniforms.uLensRadius.value;
    const lensStrengthUniform = spiralMaterial.uniforms.uLensStrength.value;
    const getAtlasChars = () => Array.from(new Set([...spiralTextChars, ...spiralSlots.map((slot) => slot.char)]));
    const rebuildGlyphAtlas = (chars) => {
        const nextAtlas = createGlyphAtlas(chars, currentFontFamily);
        nextAtlas.texture.anisotropy = Math.min(8, options.renderer.capabilities.getMaxAnisotropy());
        ...
        spiralMaterial.uniforms.uAtlasGrid.value.set(glyphAtlas.columns, glyphAtlas.rows);
        for (let i = 0; i < total; i += 1) {
            const slot = spiralSlots[i];
            const glyphIndex = glyphAtlas.glyphMap.get(slot.char) ?? fallbackGlyph;
            glyphUvArray[i * 2] = glyphIndex % glyphAtlas.columns;
    ...
    const updateSpiral = (delta, sphereState) => {
        ...
        for (let i = 0; i < lensParticles.length; i += 1) {
            ...
            const fadeIn = THREE.MathUtils.smoothstep(lifeT, 0, PLANE_LENS_FADE_IN);
            const fadeOut = 1 -
                THREE.MathUtils.smoothstep(lifeT, 1 - PLANE_LENS_FADE_OUT, 1);
            ...
            if (lens.pos.x > lensBounds)
                lens.pos.x = -lensBounds;
            if (lens.pos.x < -lensBounds)
                lens.pos.x = lensBounds;
            if (lens.pos.y > lensBounds)
                lens.pos.y = -lensBounds;
            if (lens.pos.y < -lensBounds)
                lens.pos.y = lensBounds;
        ...
        if (progressIndex !== lastProgressIndex) {
            const steps = (progressIndex - lastProgressIndex + SPIRAL_LETTER_COUNT) %
                SPIRAL_LETTER_COUNT;
            for (let step = 0; step < steps; step += 1) {
                lastProgressIndex =
                    (lastProgressIndex + 1) % SPIRAL_LETTER_COUNT;
                const slotIndex = (SPIRAL_LETTER_COUNT - lastProgressIndex) %
                    SPIRAL_LETTER_COUNT;
                ...
                glyphUvArray[slotIndex * 2 + 1] = Math.floor(glyphIndex / glyphAtlas.columns);
                glyphsUpdated = true;
            }
        }
